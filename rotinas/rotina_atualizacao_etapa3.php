<?
	...
	if($executou == true){
		$sql = "update Atualizacao set DataEtapa3 = concat(curdate(),' ',curtime()), LogUpdateMySQL = concat(LogUpdateMySQL,'\n','#".date("Y-m-d H:i:s")." -> Conclui Execução.') where IdAtualizacao = $IdAtualizacao";
		mysql_query($sql,$con);
	}
	...
	function executaQueryAtualizacao($IdAtualizacao, $IdSisbuild, $sql){
		...
		// Coódigo dos erros que serão desconsiderados
		$ErroOff = array(0,1050,1060,1062,1091);
