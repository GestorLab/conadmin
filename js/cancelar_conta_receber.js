			...
			if(xmlhttp.readyState == 4){ 
				if(xmlhttp.status == 200){
					...
					if(xmlhttp.responseText == 'false'){
						...
						document.getElementById('cpVoltarDataBase').innerHTML		=	dados;
				...
				for(i=posInicial;i<=posFinal;i=i+7){
					...
					if(temp[0] == 'CancelarContaEventual'){
	...
	function addSelect(campo,IdGrupoParametroSistema,IdParametroSistemaTemp){
		...
		url = "xml/parametro_sistema.php?IdGrupoParametroSistema="+IdGrupoParametroSistema+"&IdParametroSistema="+IdParametroSistemaTemp;
							...
							if(aux == 1){
								...
								if(document.formulario[i-3] != undefined){
									if(document.formulario[i-7].value == Codigo){
										...
										document.formulario[i-3][2].selected		=	true;
									}
								}
							}
						}else{
