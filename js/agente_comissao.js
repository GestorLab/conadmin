	function excluir(IdAgenteAutorizado,IdServico,Parcela){
		...
		if(excluir_registro() == true){
		    ...
		    //    	xmlhttp.overrideMimeType('text/xml');
   			...
   			url = "files/excluir/excluir_agente_comissao.php?IdAgenteAutorizado="+IdAgenteAutorizado+"&IdServico="+IdServico+"&Parcela="+Parcela;
					...
					if(xmlhttp.status == 200){
						if(document.formulario != undefined){
							...
							if(parseInt(xmlhttp.responseText) == 7){
								...
								url = 'cadastro_agente_autorizado_comissao.php?Erro='+document.formulario.Erro.value;
							...
							if(numMsg == 7){
								...
								for(var i=0; i<document.getElementById('tableListar').rows.length; i++){
									if(IdAgenteAutorizado+"_"+IdServico+"_"+Parcela == document.getElementById('tableListar').rows[i].accessKey){
