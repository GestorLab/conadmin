	...
	function validar(){
		...
		if(document.formulario.QtdMinima.value==0 || document.formulario.QtdMinima.value=='0,00'){
			mensagens(1);
	...
	function avancaFoto(foto){
		if(foto != ''){
			...
			tempfoto = temp[i-1];
			var cont = 0;
			//alert(document.formulario.qtdFoto.value);
			for(i=0;i<document.formulario.qtdFoto.value;i++){
				if(fotos[i] == tempfoto){
					//alert(i+" "+(document.formulario.qtdFoto.value -2));
				...
				if(xmlhttp.readyState == 4){ 
					if(xmlhttp.status == 200){
						if(xmlhttp.responseText == 'false'){		
							...
							document.formulario.IdSubGrupoProduto[1].selected = true;
						}
						// Fim de Carregando
						carregando(false);
