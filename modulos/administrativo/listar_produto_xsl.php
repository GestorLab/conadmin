		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>
				<tr class='tableListarTitle'>
					...
					<td>
						<a href="javascript:filtro_ordenar('DescricaoReduzidaProduto','text')">Nome Produto <?=compara($localOrdem,"DescricaoProduto",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
					...
					<td class="valor">
						<a href="javascript:filtro_ordenar('ValorPrecoMedio','number')">Valor Médio (<?=getParametroSistema(5,1);?>) <?=compara($localOrdem,"ValorPrecoMedio",$ImgOrdernarASC,'')?></a>
					</td>
					<td class="valor">
						<a href="javascript:filtro_ordenar('ValorPrecoUltimaCompra','number')">Valor Última Compra (<?=getParametroSistema(5,1);?>) <?=compara($localOrdem,"ValorPrecoUltimaCompra",$ImgOrdernarASC,'')?></a>
					</td>
					<td class='bt_lista' />
				</tr>
				...
				<xsl:element name="tr">
					...
					<td>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_produto.php?IdProduto=<xsl:value-of select="IdProduto"/></xsl:attribute>
							<xsl:value-of select="DescricaoProduto"/>
						</xsl:element>
					</td>
					<td>
						<xsl:element name="a">
					...
					<td class="valor">
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_produto.php?IdProduto=<xsl:value-of select="IdProduto"/></xsl:attribute>
							<xsl:value-of select='format-number(ValorPrecoMedio,"0,00","euro")'/>
						</xsl:element>
					</td>
					<td class="valor">
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_produto.php?IdProduto=<xsl:value-of select="IdProduto"/></xsl:attribute>
							<xsl:value-of select='format-number(ValorPrecoUltimaCompra,"0,00","euro")'/>
						</xsl:element>
					</td>
					<td class='bt_lista'>
				...
				<tr class='tableListarTitle'>
					...
					<td class="valor" id='tableListarMedio'><xsl:value-of select='format-number(sum(db/reg/ValorPrecoMedio),"0,00","euro")'/></td>
					<td class="valor" id='tableListarUltima'><xsl:value-of select='format-number(sum(db/reg/ValorPrecoUltimaCompra),"0,00","euro")'/></td>
					<td />
				</tr>
			</table>
		</div>
