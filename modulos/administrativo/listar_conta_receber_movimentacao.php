<?
	...
	if($filtro_descricao_servico!=''){
		...
		$filtro_sql_servico = " and Servico.IdLoja = $local_IdLoja
								and Servico.DescricaoServico like '%$filtro_descricao_servico%'";
		...
		if($filtro_id_servico!=""){
			...
			$filtro_sql_servico = "and Servico.IdLoja = $local_IdLoja and
									Servico.IdServico = $filtro_id_servicoServico.IdServico = $filtro_id_servico ";
		}
	} else if($filtro_id_servico!=""){
		$filtro_url .= "&IdServico=".$filtro_id_servico;
		$filtro_sql .= "and
							Servico.IdLoja = $local_IdLoja and
							Servico.IdServico = $filtro_id_servico ";
		$filtro_sql_servico = "and Servico.IdLoja = $local_IdLoja and Servico.IdServico = $filtro_id_servico ";
	}
	...
	if($filtro_IdServico!=''){
		...
		$filtro_sql .= " and 		
						Servico.IdLoja = $local_IdLoja and
						Servico.IdServico = '$filtro_IdServico'";
		$filtro_sql_servico = " and Servico.IdLoja = $local_IdLoja 
								and Servico.IdServico = $filtro_id_servico ";				
	}
	...
	if($filtro_tipo_impressao == 1){
		...
		echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_conta_receber_movimentacao_xsl.php$filtro_url\"?>";
		echo "<db>";
	} else{
		...
		$pdf->Ln();
		$pdf->Cell(10.6, 1.2, 'Relatório Conta Receber Movimentação Diária', 0,0, 'L',0);
		$pdf->setFont('Arial','',9);
		...
		$pdf->Cell(1.8, 0.5, 'Lanç. Fin.', 0,0, 'L',0);
	...
	$sql	=	"select
					...
					substr(Pessoa.Nome,1,$limitenome) Nome, 
					Pessoa.CPF_CNPJ, 
					ContaReceberRecebimento.IdStatus, 
					substr(Cidade.NomeCidade,1,$limitecidade) NomeCidade,
					Estado.SiglaEstado,
				...
				where
					...
					PessoaGrupoPessoa.IdLoja = ContaReceberDados.IdLoja and 
	...
	while($lin	=	mysql_fetch_array($res)){
		...
		if($query == 'true'){
			...
			if($lin[IdContaReceber] == $IdContaReceberTemp && $lin[IdContaReceberRecebimento] == $IdContaReceberRecebTemp){
				$lin[IdContaReceber] = 0;
				$lin[IdContaReceberRecebimento] = 0;
			}else{
				if($filtro_tipo_impressao == 1){
					echo "<reg>";	
					...
					$sql2 = "SELECT
							...
							LancamentoFinanceiro.Valor ValorLancamento,
							Demonstrativo.Tipo
						FROM
							LancamentoFinanceiroContaReceber,
							Demonstrativo,
							...
							LEFT JOIN Contrato 
								ON (
									LancamentoFinanceiro.IdLoja = Contrato.IdLoja 
									AND LancamentoFinanceiro.IdContrato = Contrato.IdContrato
								) 
							LEFT JOIN OrdemServico 
								ON (
									LancamentoFinanceiro.IdLoja = OrdemServico.IdLoja 
									AND LancamentoFinanceiro.IdOrdemServico = OrdemServico.IdOrdemServico
								) 
							LEFT JOIN Servico 
								ON ((
								  Servico.IdLoja = Contrato.IdLoja 
								  AND Servico.IdServico = Contrato.IdServico
								) 
								OR (
								  Servico.IdLoja = OrdemServico.IdLoja 
								  AND Servico.IdServico = OrdemServico.IdServico
								))
						WHERE
					...
					while($lin2 = mysql_fetch_array($res2)){
						echo 	"<LancamentoFinanceiro>";
						echo 	"<IdLancamentoFinanceiro><![CDATA[$lin2[IdLancamentoFinanceiro]]]></IdLancamentoFinanceiro>";
						echo 	"<ValorLancamento><![CDATA[$lin2[ValorLancamento]]]></ValorLancamento>";
						echo 	"<Tipo><![CDATA[$lin2[Tipo]]]></Tipo>";
						echo 	"</LancamentoFinanceiro>";
					}
					echo 	"</Lancamentos>";
					echo 	"<Valor>$lin[Valor]</Valor>";
					echo 	"<ValorMoraMulta>$lin[ValorMoraMulta]</ValorMoraMulta>";
					...
					$IdContaReceberTemp = $lin[IdContaReceber];
					$IdContaReceberRecebTemp = $lin[IdContaReceberRecebimento];
				} else{
					$tamanho = 1;
					$lancamento = "";
					$valorlancamento = "";
					...
					$res2 = mysql_query($sql2,$con);
					while($lin2 = mysql_fetch_array($res2)){
						$pdf->Cell(5.8,0.5,$lin2[IdLancamentoFinanceiro], 0,0, 'L',true);
					}
					
					$pdf->Cell(2.4,0.5,$lin[CPF_CNPJ], 0,0, 'L',true);
					$pdf->Cell(1.7,0.5,$lin[NumeroNF], 0,0, 'L',true);
					$pdf->Cell(4.4,0.5,$lin[NomeCidade], 0,0, 'L',true);
	...
	if($filtro_tipo_impressao == 1){
		...
		$pdf->Cell(15.1,0.5,number_format($totalvalor,2,',',''), 0,0, 'R',0);
