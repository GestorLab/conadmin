<?
	...
	$filtro_tipo_contrato		= $_POST['filtro_tipo_contrato'];
	$filtro_parametro			= $_POST['filtro_parametro'];
	$filtro_valor_parametro		= url_string_xsl($_POST['filtro_descricao_parametro'],"URL",false);
	...
	if($filtro_campo != ""){
		switch($filtro_campo){
			case 'ContratoReAtivado':
					$filtro_url .= "&Campo=".$filtro_campo;
					$filtro_sql .= " AND ContratoStatus.IdLoja = Contrato.IdLoja and
									 ContratoStatus.IdContrato = Contrato.IdContrato and
									 ContratoStatus.IdStatusAntigo IN (1,102,101) and
									 ContratoStatus.IdStatus >=200 and
									 ContratoStatus.IdStatus <= 399";
					...
					if($filtro_data_fim != ''){
						...
						$filtro_sql .= " and substring(ContratoStatus.DataAlteracao, 1, 10) <= '$filtro_data_fim'";
					}
					
					$filtro_from = " ,ContratoStatus";
					break;
			case 'PrimeiroContrato':
				$filtro_url .= "&Campo=".$filtro_campo;
				$filtro_sql .= " AND Contrato.IdContrato in (SELECT
																		MIN(IdContrato) IdContrato
																FROM
																		Contrato
																WHERE
																		IdLoja = $local_IdLoja
				...
				if($filtro_data_fim != ''){
					...
					break;
			case 'ContratoMigrado':
				$filtro_url .= "&Campo=".$filtro_campo;
				$filtro_sql .= " AND (Contrato.IdContratoMigrado != NULL OR Contrato.IdContratoMigrado != '')";
	...
	if($filtro_tipo_contrato != ""){
		$filtro_url .= "&TipoContrato=".$filtro_tipo_contrato;
		$filtro_sql .= " and Contrato.TipoContrato=".$filtro_tipo_contrato; 
	}
	...
	if($filtro_IdPessoa!=''){
		$filtro_url .= "&IdContrato=".$filtro_IdPessoa;
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_contrato_primeiro_contrato_xsl.php$filtro_url\"?>";
	echo "<db>";
	...
	while($lin	=	mysql_fetch_array($res)){
		...
		$lin[TotalDesc]		.= $lin[ValorDesconto];
		$lin[ValorDesconto] = number_format($lin[ValorDesconto],2,',','.');
		$lin[ValorTotal] 	= $lin[Valor] - $lin[ValorDesconto];
		$lin[Total]			.= $lin[ValorTotal];
		...
		echo 	"<ValorDesconto><![CDATA[$lin[ValorDesconto]]]></ValorDesconto>";
		echo 	"<ValorTemp><![CDATA[$lin[ValorTemp]]]></ValorTemp>";
		echo 	"<TotalDesc><![CDATA[$lin[TotalDesc]]]></TotalDesc>";
		...
		echo 	"<Total><![CDATA[$lin[Total]]]></Total>";
