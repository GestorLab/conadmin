...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='listar_pessoa_email.php' target='conteudo'>
			...
			<table>
				<tr>
					...
					<td><?=dicionario(191)?></td>
					<td><?=dicionario(192)?></td>
				...
				<tr>
					...
					<td>
						<select name='filtro_pessoa_possui_email' style='width:100px' onFocus="Foco(this,'in')" onBlur="Foco(this,'out')" onChange="atualizaSessao(this.name,this.value)" onKeyDown='listar(event)'>
							<option value=''><?=dicionario(153)?></option>
							<?
								$sql = "select 
											IdParametroSistema,
										...
										from
											ParametroSistema 
										where
											IdGrupoParametroSistema = 231";
								$res = @mysql_query($sql,$con);
								while($lin = @mysql_fetch_array($res)){
									echo "<option value='$lin[IdParametroSistema]' ".compara($localPessoaEmail,$lin[IdParametroSistema],"selected='selected'","").">$lin[ValorParametroSistema]</option>";
								}
							?>
						</select>
					</td>
					<td>
						<select name='filtro_pessoa_forma_cobranca' style='width:170px' onFocus="Foco(this,'in')" onBlur="Foco(this,'out')" onChange="atualizaSessao(this.name,this.value)" onKeyDown='listar(event)'>
							<option value=''><?=dicionario(153)?></option>
