<?
	...
	$filtro_ticket				= $_POST['IdTicket'];
	$filtro_ocultar_local_aber	= $_POST['filtro_ocultar_local_aber'];
	...
	if($filtro_ocultar_local_aber != ""){
		$filtro_url	.= "&OcultarLocalAbertura=".$filtro_ocultar_local_aber;
	}
	if($filtro_status!=""){
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_help_desk_pendente_atualizacao_reincidente_xsl.php$filtro_url\"?>";
	echo "<db>";
	...
	$sql	=	"select
					...
					substr(HelpDesk.Assunto,1,40) AssuntoTemp,
				...
				where
					...
					HelpDesk.IdTicket in (
						select distinct 
							HelpDeskHistorico.IdTicket 
						from
							HelpDeskHistorico 
						where 
							HelpDeskHistorico.IdStatusTicket = 300
					) and 
					(
						IdStatus = 100 or 
						IdStatus = 500
	...
	while($lin = @mysql_fetch_array($res)){
		...
		$sql3 = "select
					...
					subString(HelpDeskTipo.DescricaoTipoHelpDesk, 1, 8) DescricaoTipoHelpDeskTemp,
					HelpDeskSubTipo.DescricaoSubTipoHelpDesk,
					subString(HelpDeskSubTipo.DescricaoSubTipoHelpDesk, 1,3) DescricaoSubTipoHelpDeskTemp
				from
					HelpDeskTipo,
					HelpDeskSubTipo
				where
