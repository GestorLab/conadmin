<?php
	...
	if($filtro_somar_contrato_cancelado == 'Nao'){
		$filtro_sql .= " AND Contrato.IdStatus != 1 ";
	}
	if($filtro_somar_contrato_cancelado_migrado == 'Nao'){
		$filtro_sql .= " AND Contrato.IdStatus != 102 ";
	}
	if($filtro != 'DataCadastroPessoa'){
		$sql	=	"select 
						substring(Contrato.$filtro,1,4) Ano,
	...
	$graph->title->Set("Quantidade de Novos Contratos por Ano");
	$graph->subtitle->Set("(QTD x Ano)");
