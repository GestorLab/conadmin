<?
	...
	$filtro_IdCidade			= $_GET['IdCidadeTemp'];
	$filtro_limit				= $_POST['filtro_limit'];
	...
	if($filtro_IdCidade ==""){
		$filtro_IdCidade = $_POST['IdCidadeTemp'];
	}
	
	if($filtro_IdCidade != ""){
		$filtro_url .= "&IdCidade=$filtro_IdCidade";
		$filtro_sql .=	" and PessoaEndereco.IdCidade = $filtro_IdCidade";
	}
	...
	if($filtro_campo!=''){
		...
		switch($filtro_campo){
			...
			case 'MesFaturamento':
				if($filtro_valor!=""){
					$filtro_valor	=	dataConv($filtro_valor,'m/Y','Y-m');
					$filtro_sql    .=	" and substring(OrdemServico.DataFaturamento, 1, 7) like '$filtro_valor%'";
				}else{
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_ordem_servico_localidade_xsl.php$filtro_url\"?>";
	echo "<db>";
	...
	$sql	=	"select
				...
				where
					...
					OrdemServico.`IdPessoa` = PessoaEndereco.`IdPessoa` and
					OrdemServico.`IdPessoaEndereco` = PessoaEndereco.`IdPessoaEndereco` and
	...
	while($lin	=	@mysql_fetch_array($res)){
		...
		$lin[DataHoraTemp]  = dataConv(substr($lin[DataAgendamentoAtendimento],0,10),"Y-m-d","d/m/Y"); 
