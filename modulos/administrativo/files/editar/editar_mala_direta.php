<?
	if(!permissaoSubOperacao($localModulo,$localOperacao,"U")){
		...
		if($local_IdTipoConteudo == '1' || $local_IdTipoConteudo == '3'){
			if($local_IdTipoMensagem == '' || $local_IdTipoMensagem == 25){
				$sql = "select 
			...
			if($local_IdTipoConteudo != '2'){
				$local_IdTipoMensagemTemp = $local_IdTipoMensagem;
				$local_IdTipoMensagem = 25;
			} else{
				$sql = "select 
							IdTipoMensagem 
						from 
				...
				if($lin[IdTipoMensagem] != $local_IdTipoMensagem){
					$local_IdTipoMensagemTemp = $lin[IdTipoMensagem];
		...
		if($local_IdTipoConteudo != '3' && $local_IdTipoMensagemTemp > 999999){
			$sql = "delete from TipoMensagem where IdLoja = $local_IdLoja and IdTipoMensagem = $local_IdTipoMensagemTemp;";
			$local_transaction[$tr_i] = @mysql_query($sql, $con);
			$tr_i++;
		}
		...
		if($local_IdTipoConteudo == '1' && $_POST['AltAnexoArquivo'] == '1'){
			...
			$local_ExtArquivoAnexo	= strtolower(end(explode(".",$local_NomeOriginal)));
			...
			if(in_array($local_ExtArquivoAnexo, $local_ExtArquivoAnexoAceito)){
				...
				if($local_transaction[$tr_i]){
					...
					$LinkArquivo = getParametroSistema(6,3)."/modulos/administrativo/anexos/mala_direta/".$local_IdMalaDireta;
					...
					if($local_ExtArquivoAnexo == ".jpg"){
						...
						if(!@copy($_FILES['EndArquivoAnexo']['tmp_name'], $EnderecoArquivo)){
							@rmdir("./anexos/mala_direta/".$local_IdMalaDireta);
			...
			$tr_i++;
		} elseif($local_IdTipoConteudo != '1' && $CodeHTML == ''){
			$sql = "update MalaDireta set 
						ExtModelo = NULL
					where
						IdLoja = '$local_IdLoja' and
