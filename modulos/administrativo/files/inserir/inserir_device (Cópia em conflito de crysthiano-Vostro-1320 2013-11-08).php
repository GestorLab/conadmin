...
class Device extends Entity
{
		...
		*/
		...
		foreach($this->dataPOST['dados'] as $key => $value){
			...
			if(array_key_exists($key, $this->log)){
				...
				if($dataUrl['Tipo'] == 3){
					$logAux[] = ($value != 'NULL' && trim($value) != '') ? date("d/m/Y H:i:s") . " [{$this->dataSession['Login']}] - {$this->log[$key]}: $value\n" : '';
				}
				else{
	...
	protected function arrayDiff($dataWhere, $dataForm)
	{
		...
		if($lin != null){
			...
			$teste = array_diff_assoc($dataForm, $lin);
			print_r($teste);die;
