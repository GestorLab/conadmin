		...
		*/
		...
		foreach($this->dataPOST["atributos"] as $key => $value){
			...
			if($lin == null){
				...
				$lin = $this->findByOne('DeviceAtributo', $data);
				//print_r($value);die;
			} 
			foreach($value as $index => $val){
				...
				if($tipoMsg == 3){
					$logAux[] = ($val != 'NULL' && !strstr($index, "Id") && $val != '') ? date("d/m/Y H:i:s") . " [{$this->dataSession['Login']}] - cadastrou $DescricaoAtributo: $val\n" : '';
				}else{
					$logAux[] = ($val != 'NULL' && !strstr($index, "Id") && $val != '') ? date("d/m/Y H:i:s") . " [{$this->dataSession['Login']}] - Alterou $DescricaoAtributo: [$lin[$index] > $val]\n" : '';
				}	
