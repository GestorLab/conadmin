<?
	...
	$filtro_ocultar_local_cob			= $_POST['filtro_ocultar_local_cob'];
	$filtro_ocultar_num_conta_rec		= $_POST['filtro_ocultar_num_conta_rec'];
	...
	if($filtro_lista_contrato == '' && $_GET['ListaContrato']!=''){
		$filtro_lista_contrato = $_GET['ListaContrato'];
	}
	...
	if($filtro_ocultar_local_cob != ""){
		$filtro_url .= "&OcultarLocalCob=$filtro_ocultar_local_cob";	
	}
	
	if($filtro_ocultar_num_conta_rec != ""){
		$filtro_url .= "&OcultarNumRec=$filtro_ocultar_num_conta_rec";	
	}
	
	if($filtro_lista_contrato != ''){
		$filtro_id_pessoa = '';
		...
		$sql	=  "select 
					...
					where 
						...
						IdContrato in ($filtro_lista_contrato)";
		$resContrato	=	@mysql_query($sql,$con);
		while($linContrato	=	@mysql_fetch_array($resContrato)){
			if($filtro_id_pessoa == ''){
				$filtro_id_pessoa   =	$linContrato[IdPessoa]; 
			}else{
				$filtro_id_pessoa  .=	",".$filtro_id_pessoa; 
			}
		}
		$filtro_sql   .=	" and Pessoa.IdPessoa in ($filtro_id_pessoa)"; 
	}
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_nota_fiscal_xsl.php$filtro_url\"?>";
	echo "<db>";
