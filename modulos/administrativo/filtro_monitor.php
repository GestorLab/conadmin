...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='listar_monitor.php'>
			<input type='hidden' name='corRegRand'				value='<?=getParametroSistema(15,1)?>' />
			<input type='hidden' name='filtro' 					value='s' />
			<input type='hidden' name='filtro_ordem'			value='<?=$localOrdem?>' />
			<input type='hidden' name='filtro_ordem_direcao'	value='<?=$localOrdemDirecao?>' />
			<input type='hidden' name='filtro_tipoDado'			value='<?=$localTipoDado?>' />
			<input type='hidden' name='filtro_id_monitor'		value='<?=$localIdMonitor?>' />
			<table>
				<tr>
					<td>Descrição do Monitor </td>
					<td>Host do Endereço</td>
					<td>Status</td>
					<td>Qtd.</td>
					<td />
				</tr>
				<tr>
					...
					<td><input type='text' name='filtro_host_address' value='<?=$localHostAddress?>' style='width:303px' maxlength='100' onFocus="Foco(this,'in')" onBlur="Foco(this,'out')" onKeyDown="listar(event);" /></td>
					<td>
