<?
	...
	if($filtro_IdStatus != ''){
		$filtro_sql .= " and OrdemServico.IdStatus in ($filtro_IdStatus)";
	} elseif($filtro_idstatus!=''){
	...
	if($filtro_campo!=''){
		...
		switch($filtro_campo){
			...
			case 'LoginAtendimento':
				$sql = "SELECT
						...
						FROM
							...
							Pessoa
						WHERE
							Pessoa.IdLoja = '$local_IdLoja' AND
							Pessoa.Nome = '$filtro_valor' AND
							Usuario.IdPessoa = Pessoa.IdPessoa";
				$res = mysql_query($sql,$con);
				if(mysql_num_rows($res) > 0){
					$dados = mysql_fetch_array($res);
					$filtro_sql	.=	" and (OrdemServico.LoginAtendimento like '%$dados[Login]%')";
				}else{
			...
			case 'Responsavel':
				...
				if(mysql_num_rows($res) > 0){
					$dados = mysql_fetch_array($res);
					$filtro_sql	.=	" and (OrdemServico.LoginSupervisor like '%$dados[Login]%')";
				}else{
					$filtro_sql	.=	" and (OrdemServico.LoginSupervisor like '%$filtro_valor%')";
				}
				break;
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_ordem_servico_xsl.php$filtro_url\"?>";
	echo "<db>";
	...
	while($lin	=	@mysql_fetch_array($res)){
		...
		echo 	"<LoginSupervisor><![CDATA[$lin[LoginSupervisor]]]></LoginSupervisor>";
