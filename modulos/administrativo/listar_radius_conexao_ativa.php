<?
	...
	if($filtro_oculta_nas!="")
		$filtro_url .= "&OcultaNAS=".$filtro_oculta_nas;
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_radius_conexao_ativa_xsl.php$filtro_url\"?>";
	echo "<db>";
	...
	if($filtro_servidor != ""){
		$servidor = " and IdCodigoInterno = '$filtro_servidor'";
	}
	...
	while($lin = mysql_fetch_array($res)){
		...
		while($linRadius = @mysql_fetch_array($resRadius)){
			...
			$linRadius[DuracaoTemp]		= SubHora($linRadius[DataFimTemp],$linRadius[DataInicioTemp],'s');    
			$linRadius[DuracaoTemp]		= SegHora($linRadius[DuracaoTemp]);
			$linRadius[Duracao]			= str_replace(':','',$linRadius[DuracaoTemp]);

			$Duracao					+= horaSegundo($linRadius[DuracaoTemp]);
			...
			echo 	"<DuracaoTemp><![CDATA[$linRadius[DuracaoTemp]]]></DuracaoTemp>";
