...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='listar_contrato_parametro_assinatura.php'>
			...
			<table>
				...
				<tr>
					...
					<td>
						<select name='filtro_assinatura' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out');" style='width:100px' onKeyDown='listar(event)'>
							<option value=''><?=dicionario(153)?></option>
							<?
								$sql = "select IdGrupoCodigoInterno, IdCodigoInterno, ValorCodigoInterno from CodigoInterno where IdGrupoCodigoInterno = 9 and IdLoja = 1  order by ValorCodigoInterno";
								$res = mysql_query($sql,$con);
								while($lin = mysql_fetch_array($res)){
									$lin[ValorCodigoInterno] = url_string_xsl($lin[ValorCodigoInterno],'convert');
									echo "<option value='$lin[IdCodigoInterno]' ".compara($localAssinatura,$lin[IdCodigoInterno],"selected='selected'","").">$lin[ValorCodigoInterno]</option>";
								}
							?>
						</select>
					</td>
