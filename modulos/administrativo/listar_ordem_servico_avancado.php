<?
	...
	if($_GET['filtro_idstatus'] != ''){
		$filtro_idstatus		= $_GET['filtro_idstatus'];
	}
	
	if($_GET['filtro_grupo'] != ''){
		$filtro_grupo			= $_GET['filtro_grupo'];
	}
	...
	if($filtro_campo!=''){
		...
		switch($filtro_campo){
			...
			case 'DataCancelamento':
				if($filtro_valor!=""){
					$filtro_valor	=	dataConv($filtro_valor,'d/m/Y','Y-m-d');
					$filtro_sql    .=	"and OrdemServico.IdStatus = 0 and substring(OrdemServico.DataAlteracao, 1, 10) like '$filtro_valor%'";
				}
				break;
			case 'MesCancelamento':
				if($filtro_valor!=""){
					$filtro_valor	=	dataConv($filtro_valor,'m/Y','Y-m');
					$filtro_sql    .=	"and OrdemServico.IdStatus = 0 and substring(OrdemServico.DataAlteracao, 1, 7) like '$filtro_valor%'";
				}
				break;
		}
	}else{
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_ordem_servico_avancado_xsl.php$filtro_url\"?>";
	echo "<db>";
