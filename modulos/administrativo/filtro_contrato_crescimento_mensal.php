...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='menu_contrato_crescimento_mensal.php'>
			<input type='hidden' name='IdServicoTemp'	value='<?=$localIdServico?>' />
			<input type='hidden' name='Local'	value='ContratoCrescimentoMensal' />
			<table>
				<tr>
					<td><?=dicionario(381)?></td>
					<td><?=dicionario(384)?></td>
				...
				<tr>
					<td>
						<select name='Filtro' style='width:170px' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out');" onKeyDown="listar();"/>
					...
					<td>
						<select name='Ano' style='width:90px' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out');" onKeyDown="listar();"/>
							<?
								$aux	=	0;
								if($localFiltro != 'DataCadastroPessoa'){
									$sql	=	"select 
													Ano 
												from 
													(select 
														substring($localFiltro,1,4) Ano 
													from 
														Contrato 
													where 
														IdLoja = $localIdLoja $sqlAux
													) Ano 
												group by Ano 
												order by Ano;";
								}else{
									$sql	=	"select 
												...
												from 
													(select 
														substring(Pessoa.DataCriacao,1,4) Ano 
													from 
														Pessoa, 
														Contrato 
													where 
														Contrato.IdLoja = $localIdLoja and
														Contrato.IdLoja = Pessoa.IdLoja and 
														Contrato.IdPessoa = Pessoa.IdPessoa $sqlAux
													) Ano 
												group by Ano 
												order by Ano;";
								}
					...
					<td><input type='text' value='<?=$localDescricaoServico?>' name='DescricaoServico' style='width:260px' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out')" onKeyDown="listar(event)"/></td>
					<td><input type='submit' value='<?=dicionario(166)?>' style='width:60px' class='botao' /></td>
	...
	<div class='graficos' style='text-align:center'>
		<BR /><BR /><img src='graficos/contrato_crescimento_mensal.php?filtro=<?=$localFiltro?>&ano=<?=$localAno?>&filtro_descricao_servico=<?=$localDescricaoServico?>&filtro_id_servico=<?=$localIdServico?>&filtro_somar_contrato_cancelado=<?=$filtro_somar_contrato_cancelado?>&filtro_somar_contrato_cancelado_migrado=<?=$filtro_somar_contrato_cancelado_migrado?>'/>	
	</div>
	<script>
