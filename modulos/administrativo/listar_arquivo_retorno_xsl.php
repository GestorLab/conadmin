<?
	...
	if($localIdStatus == 1){//se status = cadastrado
		$btn_del = '<xsl:attribute name="src">../../img/estrutura_sistema/ico_del.gif</xsl:attribute>
					<xsl:attribute name="alt">Excluir?</xsl:attribute>
					<xsl:attribute name="onClick">excluir(<xsl:value-of select="IdLocalRecebimento"/>,<xsl:value-of select="IdArquivoRetorno"/>)</xsl:attribute>';
	} else{
		$btn_del = '<xsl:attribute name="src">../../img/estrutura_sistema/ico_del_c.gif</xsl:attribute>';
	}
		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>					
				<tr class='tableListarTitle'>
					...
					<td>
						<a href="javascript:filtro_ordenar('NumSeqArquivo','number')">Nº Sequencial <?=compara($localOrdem,"NumSeqArquivo",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
					...
					<td>
						<a href="javascript:filtro_ordenar('DataProcessamento','number')">Data Process. <?=compara($localOrdem,"DataProcessamento",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
						<a href="javascript:filtro_ordenar('QtdRegistro','number')">QTD. Lanç. <?=compara($localOrdem,"QtdRegistro",$ImgOrdernarASC,'')?></a>
					</td>
					<td class='valor'>
						<a href="javascript:filtro_ordenar('ValorTotal','number')">Valor Recebido (<?=getParametroSistema(5,1)?>)<?=compara($localOrdem,"ValorTotal",$ImgOrdernarASC,'')?></a>
					</td>
					<td style='width: 77px'>
				...
				<xsl:element name="tr">
					...
					<td>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_arquivo_retorno.php?IdLocalRecebimento=<xsl:value-of select="IdLocalRecebimento"/>&amp;IdArquivoRetorno=<xsl:value-of select="IdArquivoRetorno"/></xsl:attribute>
							<xsl:value-of select="NumSeqArquivo"/>
						</xsl:element>
					</td>
					<td>
						<xsl:element name="a">
					...
					<td>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_arquivo_retorno.php?IdLocalRecebimento=<xsl:value-of select="IdLocalRecebimento"/>&amp;IdArquivoRetorno=<xsl:value-of select="IdArquivoRetorno"/></xsl:attribute>
							<xsl:value-of select="DataProcessamentoTemp"/>
						</xsl:element>
					</td>
					<td>
						<xsl:element name="a">
				...
				<tr class='tableListarTitle'>
					...
					<td class='valor' id='tableListarValor'><xsl:value-of select='format-number(sum(db/reg/ValorRecebido),"0,00","euro")' /></td>
					<td colspan='2' />
				</tr>
			</table>
		</div>
