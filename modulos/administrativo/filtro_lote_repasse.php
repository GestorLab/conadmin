...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='listar_lote_repasse.php'>
			...
			<input type='hidden' name='IdLoteRepasse'			value='' />
			<table class='filtro'>
				<tr>
					<td><?=dicionario(775)?></td>
				...
				<tr>
					...
					<td>
						<select name='filtro_status' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out')" style='width:120px' onKeyDown='listar(event)'>
							...
							<?
								$sql = "select IdParametroSistema, ValorParametroSistema from ParametroSistema where IdGrupoParametroSistema=57 order by ValorParametroSistema";
