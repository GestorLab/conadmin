<?
	...
	$filtro_pagamento					= $_POST['filtro_pagamento'];
	...
	if($filtro_pagamento != ""){
		$filtro_url .= "&Pagamento=".$filtro_pagamento;
		switch($filtro_pagamento){
			case 'VencimentoOriginal':
				$filtro_sql .= " and ContaReceberDados.DataVencimento > ContaReceberRecebimento.DataRecebimento";
				$sqlAux .= "";
				break;
			case 'VencimentoAtual':
				$filtro_sql .= " and ContaReceberRecebimento.DataRecebimento <= ContaReceberDados.DataVencimento";
				break;
			case 'Vencido':
				$filtro_sql .= " and ContaReceberRecebimento.DataRecebimento > ContaReceberDados.DataVencimento";
				break;
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_conta_receber_pagamento_vencimento_xsl.php$filtro_url\"?>";
	echo "<db>";
...
*/			
		...
		/*	if($filtro_soma == 1){
			...
			$sqlVencimentoOriginal = "SELECT 
										IdContaReceber,
										MIN(DataVencimento) VencimentoOriginal 
									FROM
										ContaReceberVencimento 
									WHERE
										IdContaReceber = '$lin[IdContaReceber]'";
			$resVencimentoOriginal	= mysql_query($sqlVencimentoOriginal, $con);
			$linVencimentoOriginal  = mysql_fetch_array($resVencimentoOriginal);
			$linVencimentoOriginal[VencimentoOriginal] =	dataConv($linVencimentoOriginal[VencimentoOriginal],"Y-m-d","d/m/Y");
			echo 	"<VencimentoOriginal><![CDATA[$linVencimentoOriginal[VencimentoOriginal]]]></VencimentoOriginal>";			
