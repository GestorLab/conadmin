...
### // Cabeçalho
	...
	// Campo  8 (Constante "00000" )							[3]   77-79		N
	$Campo[8] = "00000";
	...
	while($lin = mysql_fetch_array($res)){
		...
		*/
			...
			// Campo  6 (Código do Acumulador)							[7]  38-44		N 
			$Campo[6] = $local_CodigoAcumulador;
			$Campo[6] = preenche_tam($Campo[6], 7, '');	
		
			// Campo  7 (Sigla do Estado do Cliente)					[2]  45-46		N  
			$Campo[7] = $lin[SiglaEstado];
			$Campo[7] = preenche_tam($Campo[7], 2, '');
			
			// Campo  8 (Seguimento)									[7]  47-53		N 
			$Campo[8] = str_replace(".","",$lin2[CFOP]);
			$Campo[8] = preenche_tam($Campo[8], 7, '');

			// Campo  9 (Número do Documento)							[7]  54-60		N  
			$Campo[9] = $lin[NumeroDocumento];
			$Campo[9] = preenche_tam($Campo[9], 7, '');
		
			// Campo  10 (Docto Final)									[7]  61-67		N 
			$Campo[10] = $local_DoctoFinal;
			$Campo[10] = preenche_tam($Campo[10], 7, '');
		
			// Campo  11 (Série)										[7]  68-74		X 
			$Campo[11] = $lin[Serie];
			$Campo[11] = preenche_tam($Campo[11], 7, 'X');
		
			// Campo  12 (Data de Serviço)								[10] 75-84		X 
			$Campo[12] = dataConv($lin[DataEmissao],'Y-m-d','d/m/Y');
			$Campo[12] = preenche_tam($Campo[12], 10, 'X');
		
			// Campo  13 (Data de Emissão)								[10] 85-94		X 
			$Campo[13] = dataConv($lin[DataLancamento],'Y-m-d','d/m/Y');
			$Campo[13] = preenche_tam($Campo[13], 10, 'X');
		
			// Campo  14 (Valor Contábil (13,2))						[13] 95-107		N 		
			$Campo[14] = number_format($lin[ValorTotal], 2, '.', '');
			$Campo[14] = str_replace(".","",$Campo[14]);
			$Campo[14] = preenche_tam($Campo[14], 13, '');		
			
			// Campo  15 (Reservado)									[30] 108-137	X 
			$Campo[15] = preenche_tam(" ", 30, 'X');
		
			// Campo  16 (Fato Gerador da CRF)							[1] 138-138		X 
			// ("E"=Emissão ou "P"=Pagamento)
			$Campo[16] = "E";
			$Campo[16] = preenche_tam($Campo[16], 1, 'X');
		
			// Campo  17 (Alfanumérico	Fato Gerador do IRRF)		 	[1] 139-139		X 
			// ("E"=Emissão ou "P"=Pagamento)
			$Campo[17] = "E";
			$Campo[17] = preenche_tam($Campo[17], 1, 'X');
		
			// Campo  18 (Alfanumérico	Fato Gerador da CRFOP)		 	[1] 140-140		X 
			// ("E"=Emissão ou "P"=Pagamento)
			$Campo[18] = "E";
			$Campo[18] = preenche_tam($Campo[18], 1, 'X');
		
			// Campo  19 (Alfanumérico	Fato Gerador da IRRFP)		 	[1] 141-141		X 
			// ("E"=Emissão ou "P"=Pagamento)
			$Campo[19] = "E";
			$Campo[19] = preenche_tam($Campo[19], 1, 'X');
		
			// Campo  20 (Branco)										[1] 142-142 	X 
			$Campo[20] = preenche_tam(" ", 1, 'X');
		
			// Campo  21 (Código do Município)							[7] 143-149		N 
			$Campo[21] = $lin[IdEstado].preenche_tam($lin[IdCidade], 5, '');
			$Campo[21] = preenche_tam($Campo[21], 7, '');
