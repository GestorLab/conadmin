		...
		if(strlen($ArqRetorno[0]) == 242 || strlen($ArqRetorno[0]) == 241){
			for($i=0; $i<count($ArqRetorno);$i++){
				...
				if($i > 1 && $i < (count($ArqRetorno)-2)){
					...
					$Dados[NossoNumero]	 			= trim(substr($ArqRetorno[$i],42-1,15));


					$sqlNumeroInicial = "select
											ValorLocalCobrancaParametro InicioNossoNumero
										from
											LocalCobrancaParametro
										where
											IdLoja = $local_IdLoja and
											IdLocalCobranca = $local_IdLocalRecebimento and
											IdLocalCobrancaParametro = 'InicioNossoNumero'";
					$resNumeroInicial = mysql_query($sqlNumeroInicial,$con);
					$linNumeroInicial = mysql_fetch_array($resNumeroInicial);

					$InicioNossoNumero = $linNumeroInicial[InicioNossoNumero];

					if(substr($Dados[NossoNumero],0,strlen($InicioNossoNumero)) == $InicioNossoNumero){
						$Dados[NossoNumero] = substr($Dados[NossoNumero],strlen($InicioNossoNumero),strlen($Dados[NossoNumero])-strlen($InicioNossoNumero));
						$Dados[NossoNumero] = (int)$Dados[NossoNumero]; // Configuração personalizada para a Caixa
					}

					$Dados[ValorLancamento]			= (int)substr($ArqRetorno[$i],82-1,13);
 					$Dados[ValorLancamento]			.= ".".substr($ArqRetorno[$i],94,2);
					
					$Dados[AgenciaRecebimento] 		= substr($ArqRetorno[$i],100-1,6);
					$Dados[IdMoeda] 				= substr($ArqRetorno[$i],131-1,2);
					
					$Dados[ValorTarifa]				= (int)substr($ArqRetorno[$i],199-1,13);
					$Dados[ValorTarifa]				.= ".".substr($ArqRetorno[$i],211,2);
					...
					$Dados[ValorJurosMora]			= (int)substr($ArqRetorno[$i],18-1,13);
					$Dados[ValorJurosMora]			.= ".".substr($ArqRetorno[$i],30,2);
					
					$Dados[ValorDesconto]			= (int)substr($ArqRetorno[$i],33-1,13);
					$Dados[ValorDesconto]			.= ".".substr($ArqRetorno[$i],45,2);
