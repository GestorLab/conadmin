	...
	if(strlen($ArqRetorno[0]) == 402 || strlen($ArqRetorno[0]) == 401){
		for($i=0; $i<count($ArqRetorno);$i++){
			...
			if($i > 0 && $i < (count($ArqRetorno)-1) && substr($ArqRetorno[$i],0,1) == 1){
				...
				$Dados[NossoNumero]	 			= substr($ArqRetorno[$i],117-1,10);
				$Dados[NumeroDocumento]			= substr($ArqRetorno[$i],71-1,11);

				$Dados[NossoNumeroGerado]		= number_format(substr($ArqRetorno[$i],127-1,19),0,"","");
				...
				if($linContaReceber[IdContaReceber] != '' && $Dados[NossoNumeroGerado]!= ''){
					// Verifica a necessidade de salvar o novo Nosso Número
					$sqlImpressaoBanco = "SELECT 
											ContaReceber.IdLoja,
										...
										WHERE 
											ContaReceber.IdLoja = $linContaReceber[IdLoja] AND
											ContaReceber.IdContaReceber = $linContaReceber[IdContaReceber] AND
											ContaReceber.IdLoja = LocalCobrancaParametro.IdLoja AND
											ContaReceber.IdLocalCobranca = LocalCobrancaParametro.IdLocalCobranca AND
											LocalCobrancaParametro.IdLocalCobrancaLayout = 8 AND
											LocalCobrancaParametro.IdLocalCobrancaParametro = 'LocalImpressao' AND
											LocalCobrancaParametro.ValorLocalCobrancaParametro = 1 and
											(ContaReceber.NossoNumero = '' or ContaReceber.NossoNumero is null)";
					$resImpressaoBanco = mysql_query($sqlImpressaoBanco,$con);
					if($linImpressaoBanco = mysql_fetch_array($resImpressaoBanco)){
						$sqlContaReceber = "UPDATE ContaReceber SET
												NossoNumero = '$Dados[NossoNumeroGerado]'
											WHERE
