	...
	if(strlen($ArqRetorno[0]) == 402){
		for($i=0; $i<count($ArqRetorno);$i++){
			...
			if($i > 0 && $i < (count($ArqRetorno)-1)){
				...
				$Dados[NossoNumero]	 			= (int)trim(substr($ArqRetorno[$i],63-1,10));
				$Dados[NumeroDocumento]			= (int)trim(substr($ArqRetorno[$i],63-1,10));
				...
				$Dados[ValorPago]				= (int)substr($ArqRetorno[$i],153-1,11);
				$Dados[ValorPago]				.= ".".substr($ArqRetorno[$i],164-1,2);

				$Dados[ValorTotal]				+= $Dados[ValorPago];

				// Somente para este layout tem a necesside de calcular o valor pago
				$Dados[ValorPago] = $Dados[ValorPago] - $Dados[ValorDesconto] + $Dados[ValorJurosMora];
				...
				if($Dados[NumeroDocumento] != 0 && $Dados[ValorPago] > 0){
