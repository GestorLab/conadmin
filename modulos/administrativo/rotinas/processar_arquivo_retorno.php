	...
	if(permissaoSubOperacao($localModulo,$localOperacao,"P") == false){
		...
		if($linArquivoRetorno = @mysql_fetch_array($resArquivoRetorno)){				
			...
			if(file_exists($linArquivoRetorno[EndArquivo])){
				...
				$sqlArquivoRetorno = "UPDATE ArquivoRetorno SET IdStatus = 2 WHERE IdLoja = $local_IdLoja AND IdLocalCobranca = $local_IdLocalRecebimento AND IdArquivoRetorno = $local_IdArquivoRetorno;";
