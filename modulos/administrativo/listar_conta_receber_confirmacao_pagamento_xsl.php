...
<html>	
	...
	<body  onLoad="ativaNome('Contas a Receber/Confirmação de Pagamento')">
		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>					
				<tr class='tableListarTitle'>
					...
					<td>
						<a href="javascript:filtro_ordenar('DataConfirmacao','number')">Data Conf. <?=compara($localOrdem,"DataConfirmacao",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
						<a href="javascript:filtro_ordenar('StatusConfirmacaoPagamento')">Confirmação <?=compara($localOrdem,"StatusConfirmacaoPagamento",$ImgOrdernarASC,'')?></a>
					</td>
					<td class='bt_lista' />
				</tr>
				...
				<xsl:element name="tr">
					...
					<xsl:element name="td">
						...
						<xsl:element name="a">
							<xsl:attribute name="id">vsoma_<xsl:value-of select="IdContaReceber"/></xsl:attribute>
							<xsl:attribute name="href">cadastro_conta_receber.php?IdContaReceber=<xsl:value-of select="IdContaReceber"/></xsl:attribute>
					...
					<xsl:element name="td">
						...
						<xsl:attribute name="class">valor</xsl:attribute>
						<xsl:element name="a">
						<xsl:attribute name="id">vrecebido_<xsl:value-of select="IdContaReceber"/></xsl:attribute>
							<xsl:attribute name="href">cadastro_conta_receber.php?IdContaReceber=<xsl:value-of select="IdContaReceber"/></xsl:attribute>
					...
					<xsl:element name="td">
						...
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_conta_receber.php?IdContaReceber=<xsl:value-of select="IdContaReceber"/></xsl:attribute>
							<xsl:value-of select="StatusConfirmacaoPagamento"/>
						</xsl:element>
					</xsl:element>
					<xsl:element name="td">
						<xsl:attribute name="bgcolor"><xsl:value-of select="Color"/></xsl:attribute>
						...
						<xsl:element name="img">
							...
							<xsl:choose>
								<xsl:when test="IdStatusConfirmacaoPagamento = 1"> 
								...
								<xsl:when test="IdStatusConfirmacaoPagamento = 2"> 
									<xsl:attribute name="src"><xsl:value-of select="ImgExc"/></xsl:attribute>
									<xsl:attribute name="onClick">excluir(<xsl:value-of select="IdContaReceber"/>,<xsl:value-of select="IdStatus"/>)</xsl:attribute>
								</xsl:when> 
								<xsl:otherwise>
				...
				<tr class='tableListarTitle'>
					<td id="tableListarTotal" colspan='5'>Total: <xsl:value-of select="count(db/reg)" /></td>
					<td class='valor' id='valorsoma'><xsl:value-of select='format-number(sum(db/reg/ValorSoma),"0,00","euro")' /></td>
					<td />
					<td class='valor' id='valorrecebidosoma'><xsl:value-of select='format-number(sum(db/reg/ValorRecebidoSoma),"0,00","euro")' /></td>
					<td colspan='5' />
				</tr>
			</table>
		</div>
