		...
		if(strlen($ArqRetorno[0]) == 242){
			for($i=0; $i<count($ArqRetorno);$i++){
				...
				if($i == 0){
					...
					$Dados[AgenciaCedente]	= substr($ArqRetorno[$i],54-1,4);
					$Dados[ContaCedente]	= substr($ArqRetorno[$i],60-1,11);
				...
				if($i > 1 && $i < (count($ArqRetorno)-2)){
					...
					$Dados[NossoNumero]	 			= substr($ArqRetorno[$i],47-1,8);
	 				$Dados[NossoNumero] 			= (int)substr($Dados[NossoNumero],1,15); // Configuração personalizada para a Caixa

	//				$Dados[NumeroDocumento]			= substr($ArqRetorno[$i],59-1,15);
