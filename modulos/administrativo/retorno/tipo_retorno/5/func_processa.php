		...
		if(strlen($ArqRetorno[0]) == 402){
			for($i=0; $i<count($ArqRetorno);$i++){
				...
				if($i == 0){
					...
					#Necessita do layout para pegar as posiçoes corretas Weiner 28/03/2013
					/*$Dados[AgenciaCedente]	= substr($ArqRetorno[$i],27-1,4);
					$Dados[ContaCedente]	= substr($ArqRetorno[$i],33-1,5);
