		...
		if(strlen($ArqRetorno[0]) == 402){
			for($i=0; $i<count($ArqRetorno);$i++){
				...
				if($i == 0){
					...
					## Em desenvolvimento Weiner
					#echo $Dados[AgenciaCedente]	= substr($ArqRetorno[$i],54-1,4);
					#echo "<br>".$Dados[ContaCedente]	= substr($ArqRetorno[$i],60-1,11);
				...
				if($i > 0 && $i < (count($ArqRetorno)-1)){
					...
					$Dados[NossoNumero]	 			= (int)substr($ArqRetorno[$i],71-1,10);
