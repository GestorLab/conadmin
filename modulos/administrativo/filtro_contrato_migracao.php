...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='listar_contrato_migracao.php'>
			...
			<table>
				...
				<tr>
					...
					<td><input type='text' value='<?=$localDescricaoServico?>' name='filtro_descricao_servico' style='width:160px' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out')" onKeyDown="listar(event)"/></td>
					<td>
						<select name='filtro_campo' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out')" style='width:120px'  onKeyDown="listar(event)">
							...
							<option value='DataMigracao' <?=compara($localCampo,"DataMigracao","selected='selected'","")?>><?=dicionario(344)?></option>
						</select>
					</td>
	...
	<script type='text/javascript'>
		function atualizar_campo(){
			...
			if(document.filtro.IdEstado.value != ''){
				busca_filtro_cidade(document.filtro.IdEstado.value,Number(document.filtro.IdCidade.value));
				document.filtro.IdEstado.value = '';
			}
		}
