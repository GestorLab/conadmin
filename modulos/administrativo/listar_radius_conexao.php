<?
	...
	$filtro_dia_referencia	= $_POST['filtro_dia_referencia'];
	...
	$filtro_oculta_nas		= $_SESSION["filtro_oculta_nas"];
	...
	if($filtro_dia_referencia!=""){
		$filtro_url .= "&DiaReferencia=".$filtro_dia_referencia;
	} else{
		$filtro_dia_referencia = "";
	}
	
	if($filtro_mes_referencia!=""){
		$filtro_url	.= "&MesReferencia=".$filtro_mes_referencia;
		
		$filtro_mes_referencia 		= dataConv($filtro_mes_referencia, 'm/Y','Y-m');
		
		$filtro_sql	.= " and AcctStartTime like '$filtro_mes_referencia-$filtro_dia_referencia%'";
	}
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_radius_conexao_xsl.php$filtro_url\"?>";
	echo "<db>";
