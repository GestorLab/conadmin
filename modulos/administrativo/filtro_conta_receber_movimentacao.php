...
?>
	<div id='filtroBuscar'>
		<form name='filtro' method='post' action='listar_conta_receber_movimentacao.php'>
								...
								<tr>
									...
									<td>
										<select name='filtro_grupo_pessoa' style='width:146px'  onFocus="Foco(this,'in')"  onBlur="Foco(this,'out')" onKeyDown='listar(event)'>
											<option value=''>Todos</option>
											<?
												$sql = "select IdGrupoPessoa, DescricaoGrupoPessoa from GrupoPessoa;";
												$res = @mysql_query($sql,$con);
												while($lin = @mysql_fetch_array($res)){
								...
								<tr>
									...
									<td>
										<select name='filtro_cpf_cnpj' style='width:100px' onFocus="Foco(this,'in')"  onBlur="Foco(this,'out')"  onKeyDown='listar(event)' onChange="atualizaSessao(this.name,this.value)">
											<?
												$sql = "select IdParametroSistema, ValorParametroSistema from ParametroSistema where IdGrupoParametroSistema=213 order by ValorParametroSistema DESC";
												$res = @mysql_query($sql,$con);
												while($lin = @mysql_fetch_array($res)){
													$lin[ValorParametroSistema] = url_string_xsl($lin[ValorParametroSistema],'convert');
