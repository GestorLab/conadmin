<?
	...
	$localPais			= url_string_xsl($_GET['Pais'],'url',false);
	$Limit				= $_GET['Limit'];

	if($localOrdem == ''){					$localOrdem = "Pais";		}
		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>
				<tr class='tableListarTitle'>
					<td class='id_listar'>
						<a href="javascript:filtro_ordenar('IdPais','number')"><?=dicionario(141)?><?=compara($localOrdem,"IdPais",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
				...
				<xsl:element name="tr">
					...
					<xsl:attribute name="accessKey"><xsl:value-of select="IdPais"/></xsl:attribute>
					<td class='sequencial' style='width:25px'>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_pais.php?IdPais=<xsl:value-of select="IdPais"/></xsl:attribute>
							<xsl:value-of select="IdPais"/>
						</xsl:element>
					</td>
					<td>
						<xsl:element name="a">
					...
					<td class='bt_lista'>
						<xsl:element name="img">
							...
							<xsl:attribute name="onClick">excluir(<xsl:value-of select="IdPais"/>)</xsl:attribute>
						</xsl:element>
					</td>
				</xsl:element>
				</xsl:for-each>
