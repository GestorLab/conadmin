<?
	...
	$filtro_posicao_cobranca			= $_POST['filtro_posicao_cobranca'];
	$filtro_alteracao_de				= $_POST['filtro_alteracao_de'];
	$filtro_alteracao_ate				= $_POST['filtro_alteracao_ate'];
	...
	if($filtro_posicao_cobranca!=""){
		$filtro_url .= "&IdPosicaoCobranca=".$filtro_posicao_cobranca;
		$filtro_sql .= " and ContaReceberDados.IdPosicaoCobranca = $filtro_posicao_cobranca";
	}
	
	if($filtro_alteracao_de!="" || $filtro_alteracao_ate!=""){
		$sqlAux = ", ContaReceberPosicaoCobranca , ContaReceberDados";
		$filtro_sql .= " and ContaReceberDados.IdContaReceber = ContaReceberPosicaoCobranca.IdContaReceber and ContaReceberDados.IdContaReceber = LancamentoFinanceiroContaReceber.IdContaReceber ";

	}
	
	if($filtro_alteracao_de!=""){
		$filtro_url .= "&DataLancamento=".$filtro_alteracao_de;
		$filtro_alteracao_de = dataConv($filtro_alteracao_de,'d/m/Y','Y-m-d');					
		$filtro_sql .=	" and SUBSTRING(ContaReceberPosicaoCobranca.DataAlteracao,1,10) >= '$filtro_alteracao_de'";
	}
	
	if($filtro_alteracao_ate!=""){
		$filtro_url .= "&DataVencimento=".$filtro_alteracao_ate;
		$filtro_alteracao_ate = dataConv($filtro_alteracao_ate,'d/m/Y','Y-m-d');					
		$filtro_sql .=	" and SUBSTRING(ContaReceberPosicaoCobranca.DataAlteracao,1,10) <= '$filtro_alteracao_ate'";
	}
	...
	if($filtro_tipo_relatorio == 1 || $_GET['NumeroDocumento']){
		...
		echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_conta_receber_posicao_cobranca_xsl.php$filtro_url\"?>";
		echo "<db>";
	}else{
	...
	while($lin	=	mysql_fetch_array($res)){
		...
		/*	if($filtro_soma == 1){
			...
			if($filtro_tipo_relatorio == 1 || $_GET['NumeroDocumento']){
				...
				$sql15 = "select IdParametroSistema, ValorParametroSistema from ParametroSistema where IdGrupoParametroSistema=81 and IdParametroSistema = $lin[IdPosicaoCobranca]";
				$res15 = @mysql_query($sql15,$con);
				$lin15 = @mysql_fetch_array($res15);
				$PosicaoCobranca = $lin15[ValorParametroSistema];
				#Necessário para as Posições de Cobranca que não tem valores.
				if($PosicaoCobranca == ''){
					$PosicaoCobranca = "Não Possui";
				}
				echo 	"<IdPosicaoCobranca><![CDATA[$PosicaoCobranca]]></IdPosicaoCobranca>";
