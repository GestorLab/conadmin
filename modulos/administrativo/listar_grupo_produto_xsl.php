		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>					
				<tr class='tableListarTitle'>
					<td class='id_listar'>
						<a href="javascript:filtro_ordenar('IdGrupoProduto','number')">Id<?=compara($localOrdem,"IdGrupoProduto",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
				...
				<xsl:element name="tr">
					...
					<xsl:attribute name="accessKey"><xsl:value-of select="IdGrupoProduto"/></xsl:attribute>
					<td class='sequencial' style='width:25px'>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_grupo_produto.php?IdGrupoProduto=<xsl:value-of select="IdGrupoProduto"/></xsl:attribute>
							<xsl:value-of select="IdGrupoProduto"/>
						</xsl:element>
					</td>
					<td>
						<xsl:element name="a">
					...
					<td class='bt_lista'>
						<xsl:element name="img">
							...
							<xsl:attribute name="onClick">excluir(<xsl:value-of select="IdGrupoProduto"/>)</xsl:attribute>
						</xsl:element>
					</td>
				</xsl:element>
				</xsl:for-each>
...
<script>
	addParmUrl("marGrupoProduto","IdGrupoProduto",'<?=$localIdGrupoProduto?>');
	addParmUrl("marSubGrupoProduto","IdGrupoProduto",'<?=$localIdGrupoProduto?>');
	addParmUrl("marSubGrupoProdutoNovo","IdGrupoProduto",'<?=$localIdGrupoProduto?>');
