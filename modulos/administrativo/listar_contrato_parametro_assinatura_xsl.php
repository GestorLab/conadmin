<?
	...
	$localAssinatura				= $_GET['Assinatura'];	
	$Limit							= $_GET['Limit'];	
...
<html>	
	<head>
		...
		<script type = 'text/javascript' src = 'js/contrato_parametro_assinatura.js'></script>
	</head>
	<body  onLoad="ativaNome('<?=dicionario(346)?>')">
		<? include("filtro_contrato_parametro_assinatura.php"); ?>
		...
		<div id='conteudo'>
			<table class='tableListar' id='tableListar'  cellspacing='0'>
				<tr class='tableListarTitle'>
					...
					<td>
						<a href="javascript:filtro_ordenar('Assinatura')"><?=dicionario(348)?><?=compara($localOrdem,"Assinatura",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
				...
				<xsl:element name="tr">
					...
					<td>						
						...
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_contrato.php?IdContrato=<xsl:value-of select="IdContrato"/></xsl:attribute>
							<xsl:value-of select="AssinaturaContrato"/>
						</xsl:element>
					</td>
					<td>						
						<xsl:attribute name="bgcolor"><xsl:value-of select="Color"/></xsl:attribute>
