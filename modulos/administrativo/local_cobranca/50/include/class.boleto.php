...
class Boleta extends FPDF
{
	...
	function Demonstrativo($IdLoja, $IdContaReceber, $con){
		global $Path;
		include($Path."modulos/administrativo/local_cobranca/50/funcao_demonstrativo_pdf.php");
	}
	
	function DemonstrativoCarne($IdLoja, $IdCarne, $con){
		global $Path;
		include($Path."modulos/administrativo/local_cobranca/50/funcao_demonstrativo_carne_pdf.php");
	}
	...
	include($Path."modulos/administrativo/local_cobranca/50/include/vars_santander_banespa.php");
	$contador_contrato		=	0;
	...
	function DemonstrativoVerso($IdLoja, $IdContaReceber, $con){
		...
		$sql = "SELECT 
				...
				WHERE
					IdLocalCobrancaLayout = 50 AND
					IdLocalCobrancaParametro = 'ImprimirVerso'";
		$res = mysql_query($sql,$con);
		$linParametroLayout = mysql_fetch_array($res);
		
		if($linParametroLayout[ImprimirVerso] == 'S'){		
			include($Path."modulos/administrativo/local_cobranca/50/funcao_demonstrativo_verso_pdf.php");
		}
	}
