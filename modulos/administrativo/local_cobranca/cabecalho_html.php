		...
		if($linNotaFiscal = @mysql_fetch_array($resNotaFiscal)){	
			...
			if($linDadosPessoaNotaFiscal = mysql_fetch_array($res)){
				...
				if($linDadosPessoaNotaFiscal[Telefone1] != ''){
					if($linDadosPessoaNotaFiscal[Fax] != ''){
						$linDadosPessoaNotaFiscal[Telefone] = $linDadosPessoaNotaFiscal[Telefone1]." / ".$linDadosPessoaNotaFiscal[Fax];
					}else{
						$linDadosPessoaNotaFiscal[Telefone] = $linDadosPessoaNotaFiscal[Telefone1];
					}
				}else{
					if($linDadosPessoaNotaFiscal[Fax] != ''){
						$linDadosPessoaNotaFiscal[Telefone] = $linDadosPessoaNotaFiscal[Telefone2]." / ".$linDadosPessoaNotaFiscal[Fax];
					}else{
				...
				if($linDadosPessoaNotaFiscal[Telefone] != ''){
					if($linDadosPessoaNotaFiscal[Fax] != ''){
						...
						$linDadosPessoaNotaFiscal[Fone] 				= " - Fone: ".$linDadosPessoaNotaFiscal[Telefone];
					}
				}
