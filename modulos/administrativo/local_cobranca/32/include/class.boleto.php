...
class Boleta extends FPDF
{
	...
	function DemonstrativoCarnePersonalizado($IdLoja, $IdCarne, $con){
		global $Path;
		include($Path."modulos/administrativo/local_cobranca/32/funcao_demonstrativo_carne_pdf.php");
	}
	...
	function Titulo($IdLoja, $IdContaReceber, $con){
		...
		// L1
		if($Background == 's'){
			$PatchImagens = $Path."/modulos/administrativo/local_cobranca/4/";
		}
	...
	function TituloCarne($IdLoja, $IdContaReceber, $con){
		...
		if($Background == 's'){
			$PatchImagens = $Path."/modulos/administrativo/local_cobranca/32/";
		}
