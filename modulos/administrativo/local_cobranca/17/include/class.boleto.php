...
class Boleta extends FPDF
{
	...
	function Titulo($IdLoja, $IdContaReceber, $con){
		...
		$dadosboleto["conta"]					= $CobrancaParametro[Conta];
		...
		// L1
		if($Background == 's'){
			$PatchImagens = $Path."/modulos/administrativo/local_cobranca/17/";
		}

	    $this->Image($PatchImagens."imagens/logoreal.jpg",11,($posTemp+0.5),27.5,5,jpg);
		$this->Cell(40,6,'','T');
		...
		// L11
		$this->Ln();
	    $this->Image($PatchImagens."imagens/logoreal.jpg",11,($posTemp + 37.5),27.5,5,jpg);
		$this->Cell(40,6,'','');
		$this->SetFillColor(0,0,0);
		$this->Cell(0.4,6,'','',0,'',1);
		$this->SetFont('Arial','B',12);
		...
		$this->SetFont('Arial','B',7.5);
		$this->Cell(131.3,2.5,$Instrucoes[0],'',0,'L',0);
