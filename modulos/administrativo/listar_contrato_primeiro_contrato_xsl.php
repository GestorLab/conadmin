<?
	...
	$local_ValorParametro				= $_GET['ValorParametroServico'];
	$local_DescricaoServico				= url_string_xsl($_GET['DescricaoServico'],'URL',false);
	$local_Campo						= $_GET[$filtro_campo];
	$Limit								= $_GET['Limit'];
	$local_IdPessoa						= $_GET['IdPessoa'];
	$local_IdContrato					= $_GET['IdContrato'];
	$local_TipoPessoa					= $_GET['TipoPessoa'];
	$local_TipoContrato					= $_GET['TipoContrato'];
	$local_Parametro					= $_GET['DescricaoParametroServico'];
...
<html>	
	...
	<body  onLoad="ativaNome('<?=dicionario(900)?>')">
		<? include("filtro_contrato_primeiro_contrato.php"); ?>
		...
		<div id='conteudo'>
			<table class='tableListar' id='tableListar'  cellspacing='0'>
				<tr class='tableListarTitle'>
					...
					<td style='width: 60px'>
						<a href="javascript:filtroOrdenar('DiaCobranca','text')"><?=dicionario(293)?>.<?=compara($localOrdem,"DiaCobranca",$ImgOrdernarASC,'')?></a>
					</td>
					<td style='width: 60px'>
						<a href="javascript:filtroOrdenar('Tipo','text')"><?=dicionario(82)?>.<?=compara($localOrdem,"Tipo",$ImgOrdernarASC,'')?></a>
					</td>
					<td style='width: 76px'>
					...
					<td style='width: 76px'>
						<a href="javascript:filtroOrdenar('AbreviacaoNomeLocalCobranca','number')"><?=dicionario(285)?>.<?=compara($localOrdem,"AbreviacaoNomeLocalCobranca",$ImgOrdernarASC,'')?></a>
					</td>
					<td class='valor' style='width: 58px'>
					...
					<td style='width: 76px' class="valor">
						<a href="javascript:filtroOrdenar('ValorDesconto','number')"><?=dicionario(286)?>.<?=compara($localOrdem,"ValorDesconto",$ImgOrdernarASC,'')?></a>
					</td>
					<td style='width: 76px' class='valor'>
						<a href="javascript:filtroOrdenar('ValorTotal','number')"><?=dicionario(206)?>.<?=compara($localOrdem,"ValorTotal",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
				...
				<tr class='tableListarTitle'>
					...
					<td class='valor' id='tableListarValor'><xsl:value-of select='format-number(sum(db/reg/TotalDesc),"0,00","euro")' /></td>
					<td class='valor' id='tableListarValor'><xsl:value-of select='format-number(sum(db/reg/Total),"0,00","euro")' /></td>
					<td colspan='3'/>
				</tr>
			</table>
			<table>
