<?
	...
	function get_Conta_Receber(){
		...
		$cont	=	0;	
		$sql	=	"select
						ContaReceberDados.IdLoja, #Não e necessario
						ContaReceberDados.IdContaReceber,
						LancamentoFinanceiroDados.IdLancamentoFinanceiro, #Não e necessario
						LancamentoFinanceiroDados.IdProcessoFinanceiro,
						LancamentoFinanceiroDados.Tipo,
						LancamentoFinanceiroDados.IdContrato Codigo,
					...
					where
						...
						LancamentoFinanceiroDados.IdContaReceber = ContaReceberDados.IdContaReceber $where
                	/*group by
                    	ContaReceberDados.IdContaReceber  */
					order by
						LancamentoFinanceiroDados.IdPessoa,Tipo,IdContrato,IdLancamentoFinanceiro";
		$res	=	@mysql_query($sql,$con);
		echo mysql_error();
		echo mysql_error();
		while($lin	=	@mysql_fetch_array($res)){
			...
			/*	$sql13 = "SELECT 
							COUNT(NotaFiscal.IdNotaFiscal) > 0 CancelarNotaFiscalRecebimento
						FROM 
							NotaFiscal,
							LancamentoFinanceiroDados,
							Contrato
				...
				$sql14 = "SELECT 
							COUNT(NotaFiscal.IdNotaFiscal) > 0 CancelarNotaFiscal
						FROM 
							NotaFiscal
						WHERE 
							NotaFiscal.IdLoja = '$IdLoja' AND 
				...
				if($lin16[IdStatus] !=''){
					$lin16[IdStatus] = getParametroSistema(143,$lin16[IdStatus]);
				}
				
				$sql17 = "SELECT
								ContaReceber.IdCartao IdCartao,
								ContaReceber.IdContaDebito IdContaDebito
							FROM
								ContaReceber
							WHERE
								ContaReceber.IdLoja = '$IdLoja' AND
				...
				$dados	.=	"\n<IdCartao><![CDATA[$lin17[IdCartao]]]></IdCartao>";
				$dados	.=	"\n<IdContaDebito><![CDATA[$lin17[IdContaDebito]]]></IdContaDebito>";
				...
				$dados	.=	"\n<SerieNF><![CDATA[$lin16[Serie]]]></SerieNF>";
				$dados	.=	"\n<BaseCalculoNF><![CDATA[$lin16[ValorBaseCalculoICMS]]]></BaseCalculoNF>";
				$dados	.=	"\n<IdStatusNF><![CDATA[$lin16[IdStatus]]]></IdStatusNF>";
				...
				$dados	.=	"\n<DataAtual><![CDATA[".date("Y-m-d")."]]></DataAtual>";
				$sql17 = "select 
							*							
						from 
							ContaReceber
				...
				$res17 = @mysql_query($sql17, $con);
				$lin17 = @mysql_fetch_array($res17);
				$dados	.=	"\n<NossoNumero><![CDATA[$lin17[NossoNumero]]]></NossoNumero>";
				$cont++;
