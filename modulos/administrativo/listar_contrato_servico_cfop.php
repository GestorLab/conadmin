<?
	...
	$filtro_id_cfop					= $_POST['filtro_id_cfop'];
	$filtro_nome_cfop				= url_string_xsl($_POST['filtro_nome_cfop'],'url',false);
	...
	$filtro_nota_fiscal_cda			= $_SESSION["filtro_contrato_nota_fiscal_cda"];	
	...
	if($filtro_IdContaReceber!=''){
		$filtro_sql .= " and Contrato.IdContrato in (select distinct IdContrato from LancamentoFinanceiroDados where IdLoja = $local_IdLoja and	IdContaReceber = $filtro_IdContaReceber)";
	}
	...
	if($filtro_nome_cfop =="" && $filtro_id_cfop =="" && strtolower($filtro_nota_fiscal_cda)=="null"){
		$filtro_from .= ", CFOP";
		//if($filtro_id_servico!=""){
			$sql="	select 
					  * 
			...
			if(mysql_num_rows >= 1){
				$filtro_sql 	  .= " and Contrato.CFOP is null and ServicoCFOP.CFOP is not null and CFOP.NaturezaOperacao ='' and Contrato.NotaFiscalCDA IS NULL";
			}else{
					$left_join_servico = " 	left join ServicoCFOP on
											( 
												ServicoCFOP.IdLoja = '$local_IdLoja' and
												Servico.IdLoja = ServicoCFOP.IdLoja	and
												Servico.IdServico = ServicoCFOP.IdServico
											)";
					$filtro_sql 	  .= " and Contrato.CFOP is not null and ServicoCFOP.CFOP is null and Contrato.NotaFiscalCDA IS NULL";
			}
		/*}else{
			$left_join_servico = " 	left join ServicoCFOP on
											( 
			...
			$filtro_sql 	  .= " and ServicoCFOP.CFOP is null and Contrato.NotaFiscalCDA IS NULL";
		}*/
	}else{
		if($filtro_id_cfop!=""){
			$filtro_url 	  .= "&CFOP=".$filtro_id_cfop;
			$left_join_servico = " 	left join ServicoCFOP on
									( 
										ServicoCFOP.IdLoja = '$local_IdLoja' and
										Servico.IdLoja = ServicoCFOP.IdLoja	and
			...
			$filtro_sql 	  .= " and Contrato.CFOP = '$filtro_id_cfop' and ServicoCFOP.CFOP is not null";
		}else{
			//if($filtro_id_servico!=""){
				$sql="	select 
						  * 
				...
				$res = mysql_query($sql,$con);
				if(mysql_num_rows >= 1){
					$filtro_sql 	  .= " and Contrato.CFOP is null";
				}else{
						$left_join_servico = " 	left join ServicoCFOP on
												( 
													ServicoCFOP.IdLoja = '$local_IdLoja' and
						...
						$filtro_sql 	  .= " and ServicoCFOP.CFOP is null";
				}
			/*}else{	
				$left_join_servico = " 	left join ServicoCFOP on
											( 
		...
		if($filtro_nome_cfop!=""){
			$filtro_url .= "&NomeCFOP=".$filtro_nome_cfop;
			$filtro_from .= ", CFOP";
			$left_join_servico = " 	left join ServicoCFOP on
									( 
										ServicoCFOP.IdLoja = '$local_IdLoja' and
			...
			$filtro_sql .= " and Contrato.CFOP = CFOP.CFOP and CFOP.NaturezaOperacao like '%$filtro_nome_cfop%' and ServicoCFOP.CFOP is not null";
		}
		
		if($filtro_nota_fiscal_cda!=""){
			if(strtolower($filtro_nota_fiscal_cda)=="null"){
				$filtro_sql .= " and Contrato.NotaFiscalCDA is null";
			}else{
				$filtro_sql .= " and Contrato.NotaFiscalCDA = '$filtro_nota_fiscal_cda'";
			}
		}
	}
	/*
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_contrato_servico_cfop_xsl.php$filtro_url\"?>";
	echo "<db>";
