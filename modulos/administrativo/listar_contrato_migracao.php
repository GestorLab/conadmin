<?
	...
	$filtro_bairro				= $_POST['filtro_bairro'];
	$filtro_endereco			= $_POST['filtro_endereco'];
	...
	if($filtro_IdStatus!=""){
		$filtro_sql .= " and Contrato.IdStatus > $filtro_IdStatus";
	}	
		...
		switch($filtro_campo){
			...
			case 'DataBase':
				...
				if($filtro_IdStatus != '' && $filtro_data_fim != ''){
					...
					$filtro_sql .=	" and (Contrato.DataBaseCalculo <= '$filtro_data_fim' or Contrato.DataBaseCalculo is null)";
				} else {
					if($filtro_data_fim != ''){
						$filtro_data_fim = dataConv($filtro_data_fim,'d/m/Y','Y-m-d');
						
						$filtro_sql .=	" and (Contrato.DataBaseCalculo <= '$filtro_data_fim')";
					}
				}
				break;
			case 'DataTermino':
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_contrato_migracao_xsl.php$filtro_url\"?>";
	echo "<db>";
