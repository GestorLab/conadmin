	...
	function quantidadeDias(DataInicio,DataFim){
		var Meses = new Array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
		var DataInicio = DataInicio.split("/");
		var DataFim = DataFim.split("/");	 
		var DiaIni  = DataInicio[0];
		var MesIni  = DataInicio[1];
		var AnoIni = DataInicio[2];
		var DiaFim  = DataFim[0];
		var MesFim = DataFim[1];
		var AnoFim = DataFim[2];
		var DataInicioAux = Meses[MesIni-1]+" "+DiaIni+", "+AnoIni;
		var DataFimAux = Meses[MesFim-1]+" "+DiaFim+", "+AnoFim;
		var DiferencaDias =(Math.round((Date.parse(DataFimAux)-Date.parse(DataInicioAux))/(24*60*60*1000))*1);	
		document.formulario.qtdDias.value = DiferencaDias+1;		
	}
