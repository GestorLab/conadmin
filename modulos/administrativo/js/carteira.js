	function excluir(IdAgenteAutorizado,IdCarteira){
		...
		if(excluir_registro() == true){
		    ...
		    //    	xmlhttp.overrideMimeType('text/xml');
   			...
   			url = "files/excluir/excluir_carteira.php?IdAgenteAutorizado="+IdAgenteAutorizado+"&IdCarteira="+IdCarteira;
					...
					if(xmlhttp.status == 200){
						if(document.formulario != undefined){
							...
							if(parseInt(xmlhttp.responseText) == 7){
								...
								url = 'cadastro_carteira.php?Erro='+document.formulario.Erro.value;
							...
							if(numMsg == 7){
								...
								for(var i=0; i<document.getElementById('tableListar').rows.length; i++){
									if(IdAgenteAutorizado+"_"+IdCarteira == document.getElementById('tableListar').rows[i].accessKey){
