...
//		alert(document.formulario.IdStatus.value+" == 0");
	...
	function verificar_valor() {
		var Valor = document.formulario.Valor.value.replace(/\./, '');
		Valor = parseFloat(Valor.replace(/,/, '.'));
		var ValorRepasseTerceiro = document.formulario.ValorRepasseTerceiro.value.replace(/\./, '');
		ValorRepasseTerceiro = parseFloat(ValorRepasseTerceiro.replace(/,/, '.'));
		
		if(Valor < ValorRepasseTerceiro) {
			mensagens(145);
			return false;
		} else{
			return true;
