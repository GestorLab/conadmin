	function excluir(IdTipo){
		...
		if(excluir_registro() == true){
		    ...
		    //    	xmlhttp.overrideMimeType('text/xml');
   			...
   			url = "files/excluir/excluir_tipo_help_desk.php?IdTipo="+IdTipo;
					...
					if(xmlhttp.status == 200){
						if(document.formulario != undefined){
							...
							if(parseInt(xmlhttp.responseText) == 7){
								...
								url = 'cadastro_tipo_help_desk.php?Erro='+document.formulario.Erro.value;
							...
							if(numMsg == 7){
								...
								for(var i=0; i<document.getElementById('tableListar').rows.length; i++){
									if(IdTipo == document.getElementById('tableListar').rows[i].accessKey){
	...
	function validar(){
		if(document.formulario.DescricaoTipo.value == ''){
			mensagens(1);
			document.formulario.DescricaoTipo.focus();
			return false;
		}
