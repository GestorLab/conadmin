	...
	function busca_grupo_usuario(IdGrupoUsuario, Erro, Local){
		...
		if(document.formulario != undefined){
			if(document.formulario.Local.value == "ServicoParametro"){
				url += "&OrdemServico=1";
			}
		}
			...
			if(xmlhttp.responseText == 'false'){
				...
				switch(Local){
					case 'GrupoUsuario':
						...
						busca_usuario_grupo_usuario("",false);
						
						document.formulario.OrdemServico.value 		= '';
				...
				switch(Local){
					case 'GrupoUsuario':
						busca_usuario_grupo_usuario(IdGrupoUsuario,false);
						
						nameNode = xmlhttp.responseXML.getElementsByTagName("OrdemServico")[0]; 
						nameTextNode = nameNode.childNodes[0];
						var OrdemServico = nameTextNode.nodeValue;
						
						nameNode = xmlhttp.responseXML.getElementsByTagName("LoginSupervisor")[0]; 
						nameTextNode = nameNode.childNodes[0];
						...
						document.formulario.OrdemServico.value		= OrdemServico;
						document.formulario.DataCriacao.value 		= dateFormat(DataCriacao);
						document.formulario.LoginSupervisor.value 	= LoginSupervisor;
						document.formulario.LoginCriacao.value 		= LoginCriacao;
						document.formulario.DataAlteracao.value 	= dateFormat(DataAlteracao);
						document.formulario.LoginAlteracao.value	= LoginAlteracao;
						document.formulario.Acao.value 				= 'alterar';
						busca_login_supervisor(IdGrupoUsuario,document.formulario.LoginSupervisor);
