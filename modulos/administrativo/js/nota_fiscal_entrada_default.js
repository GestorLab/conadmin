	...
	function busca_nota_fiscal_entrada(NumeroNF, IdFornecedor, SerieNF, IdMovimentacaoProduto, Erro, Local){
		...
		if(IdMovimentacaoProduto == ''){
			...
			if(document.formulario.CPF_CNPJ.value == '' || document.formulario.CPF_CNPJ == undefined){	
				return false;
			}
		...
		if(IdMovimentacaoProduto != ''){
			if(NumeroNF  == '' && SerieNF  == '' && IdFornecedor == ''){
	    		url = "xml/nota_fiscal_entrada.php?IdMovimentacaoProduto="+IdMovimentacaoProduto;
	    	}else{
	    		url = "xml/nota_fiscal_entrada.php?NumeroNF="+NumeroNF+"&IdFornecedor="+IdFornecedor+"&SerieNF="+SerieNF;
	    	}
	    }else{	
			//alert('entroud');					
	    	url = "xml/nota_fiscal_entrada.php?NumeroNF="+NumeroNF+"&IdFornecedor="+IdFornecedor+"&SerieNF="+SerieNF+"&IdMovimentacaoProduto="+IdMovimentacaoProduto;	    	
	    }	
			...
			if(xmlhttp.readyState == 4){ 
				if(xmlhttp.status == 200){
					...
					//	if(IdMovimentacaoProduto != ''){
							...
							while(document.getElementById('tabelaNumeroSerie').rows.length > 1){
								document.getElementById('tabelaNumeroSerie').deleteRow(1);
							}
						...
						addParmUrl("marNotaFiscalEntrada","IdMovimentacaoProduto",'');
						...
						busca_fornecedor(IdFornecedor,false,Local);
						busca_nota_fiscal_produto(IdMovimentacaoProduto,false,Local);
						
						addParmUrl("marNotaFiscalEntrada","IdMovimentacaoProduto",IdMovimentacaoProduto);
					}
						...
						for(var i=0; i<xmlhttp.responseXML.getElementsByTagName("IdMovimentacaoProduto").length; i++){
							...
							for(ii=posIni;ii<=posFim;ii=ii+9){	
								if(document.formulario[ii].name.substr(0,10) == 'IdProduto_'){
									...
									if(trim(id[1]) == trim(document.formulario.QtdProduto.value)){
