...
function permissaoParamentroSistema(){
	var lista 	= new Array();

	lista[0] = "4,7"; // IdPessoa CNTSistemas
	lista[1] = "4,8"; // IdContrato CNTSistemas

	var i=0;
	while(i<lista.length){
		valorLista = lista[i].split(",");
		if(valorLista[0] == document.formulario.IdGrupoParametroSistema.value && valorLista[1] == document.formulario.IdParametroSistema.value && document.formulario.permisssaoParametroSistemaUsuario.value != 'root'){
			document.formulario.permisssaoParametroSistema.value = 1;
		break;
		}else{
			document.formulario.permisssaoParametroSistema.value = 2;
		}
		i++;
	}
}