	function excluir(IdMovimentacaoProduto){
		if(excluir_registro() == true){
		    ...
		    //    	xmlhttp.overrideMimeType('text/xml');
   			...
   			url = "files/excluir/excluir_nota_fiscal_entrada.php?IdMovimentacaoProduto="+IdMovimentacaoProduto;
					...
					if(xmlhttp.status == 200){
						if(document.formulario != undefined){
							...
							if(parseInt(xmlhttp.responseText) == 7){
								...
								url = 'cadastro_nota_fiscal_entrada.php?Erro='+document.formulario.Erro.value;
	...
	function validar(){
		...
		var achouSerial 	= 0;
		var naoAchouSerial 	= 0;
		var posi 			= 0;
		for(i=posIni;i<=posFim;i=i+9){
			//alert(document.formulario[i].name);
			...
			if(document.formulario[i+3].value==''){
				...
				qtdAux	= document.formulario[i+3].value.replace(',','.');
				qtd 	= parseFloat(qtdAux);
				qtd 	= Math.ceil(qtd);
			}
			...
			for(ii=0;ii<document.formulario.length;ii++){
				//alert('Tam '+document.formulario.length+" pos "+ii+" "+document.formulario[ii].name);
			...
			if(posIni2!=0){
				for(ii=posIni2;ii<=posFim2;ii=ii+3){				
					...
					if(temp2[1] == CodProduto){
						if(document.formulario[ii].value == 1){
							...
							SerieAux	=	document.formulario[ii+2].value;
							serie		=	SerieAux.split('\n');
							...
							if(document.formulario[ii+1].value == 1){							
										...
										if(xmlhttp.readyState == 4){ 
											if(xmlhttp.status == 200){
												if(xmlhttp.responseText != 'false'){															
													mensagens(98);
													document.formulario[posProd].focus();
													document.formulario.validar.value = 'false';
													document.formulario.submit = false;
													return false;
												}
											}
										} 				
								...
								if(naoAchouSerial == 1 && qtdTemp != 0){									
									posi = i;
									achouSerial = 1;
								}	
								if(achouSerial == 1 && qtdTemp == 0){
									naoAchouSerial = 1;
									posi = i;									
								}	
								if(qtdTemp != 0){
				...
				if(achouSerial == 1 && naoAchouSerial == 1){
					mensagens(96);
					document.formulario[posi].focus();
					return false;
				}	
			}
		}
	...
	function addNumeroSerie(id){
		...
		c0.innerHTML = "<input type='hidden' name='NSerie_"+id+"' value=''><input type='hidden' name='NSerieObrigatorio_"+id+"' value=''><textarea name='NumeroSerie_"+id+"' style='width: 327px;' rows=5 onFocus=\"Foco(this,'in')\"  onBlur=\"Foco(this,'out');\" onChange='validarNumeroSerie()'></textarea>";	
	...
	function validarNumeroSerie(){	
		...
		for(i=posIni;i<=posFim;i=i+9){
			if(document.formulario[i].name.substr(0,10) == 'IdProduto_'){
				...
				if(temp[1] == id){
					...
					qtd 	= Math.ceil(qtd);
					var posProduto = i;
					var IdProduto  = document.formulario[i].value;
				}
			}
		}
		...
		for(i=posIni;i<=posFim;i=i+3){
			...
			if(temp[1] == id){
				if(document.formulario[i].value!=""){
					SerieAux	=	document.formulario[i].value;
					serie		=	SerieAux.split('\n');
					tam			=	parseInt(serie.length);
					qtdTemp		=	0;
					qtdTemp2	=	0;
					...
					if(qtdTemp != qtdTemp2){
						...
						xmlhttp.onreadystatechange = function(){ 
							if(xmlhttp.readyState == 4){ 
								if(xmlhttp.status == 200){
									if(xmlhttp.responseText != 'false'){
										alert("Atenção! Número de série já cadastrado.");
										document.formulario[posAux].focus();
									}else{
