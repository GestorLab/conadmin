<?
	...
	$localDescricaoMonitor	= url_string_xsl($_GET['DescricaoMonitor'],'');
	$localHostAddress		= url_string_xsl($_GET['HostAddress'],'');
...
<html>	
	...
	<body  onLoad="ativaNome('Logomarca')">
		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>
				<tr class='tableListarTitle'>
					...
					<td>
						<a href="javascript:filtro_ordenar('HostAddress','text')">Host<?=compara($localOrdem,"HostAddress",$ImgOrdernarASC,'')?></a>
					</td>
					<td>
				...
				<xsl:element name="tr">
					...
					<xsl:attribute name="accessKey"><xsl:value-of select="IdMonitor"/></xsl:attribute>
					<td class='sequencial' style='width:25px'>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_monitor.php?IdMonitor=<xsl:value-of select="IdMonitor"/></xsl:attribute>
					...
					<td class='bt_lista'>
						<xsl:element name="img">
							...
							<xsl:attribute name="onClick">excluir(<xsl:value-of select="IdMonitor"/>)</xsl:attribute>
