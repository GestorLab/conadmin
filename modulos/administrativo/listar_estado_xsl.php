<?
	...
	$localSiglaEstado	= url_string_xsl($_GET['SiglaEstado'],'');
	$Limit				= $_GET['Limit'];

	if($localOrdem == ''){					$localOrdem = "Estado";		}
...
<html>	
	...
	<body  onLoad="ativaNome('<?=dicionario(15)?>')">
		...
		<div id='conteudo'>
			<table class='tableListar' cellspacing='0' Id='tableListar'>
				<tr class='tableListarTitle'>
					...
					<td>
						<a href="javascript:filtro_ordenar('SiglaEstado','text')"><?=dicionario(815)?> <?=compara($localOrdem,"SiglaEstado",$ImgOrdernarASC,'')?></a>
					</td>
					<td class='bt_lista' />
				</tr>
				<xsl:for-each select="db/reg">
				<xsl:sort order="<?=$localOrdemDirecao?>" select="<?=$localOrdem?>" />
				<xsl:element name="tr">
					<xsl:attribute name="class">tableListarDados</xsl:attribute>
					<xsl:attribute name="onmouseover">destacaRegistro(this,true)</xsl:attribute>
					<xsl:attribute name="onmouseout">destacaRegistro(this,false)</xsl:attribute>
					<xsl:attribute name="accessKey"><xsl:value-of select="IdPais"/>_<xsl:value-of select="IdEstado"/></xsl:attribute>
					<td class='sequencial' style='width:25px'>
						<xsl:number value="position()" format="1" />
					</td>
					<td style='width: 235px'>
						<xsl:element name="a">
							<xsl:attribute name="href">cadastro_estado.php?IdPais=<xsl:value-of select="IdPais"/>&amp;IdEstado=<xsl:value-of select="IdEstado"/></xsl:attribute>
					...
					<td class='bt_lista'>
						<xsl:element name="img">
							...
							<xsl:attribute name="onClick">excluir(<xsl:value-of select="IdPais"/>,<xsl:value-of select="IdEstado"/>)</xsl:attribute>
						</xsl:element>
					</td>
				</xsl:element>
				</xsl:for-each>
...
<script>
	...
	<?
		if($localIdPais != ''){
			echo"verifica_estado($localIdPais)";
		}
	?>
</script>
</xsl:template>
