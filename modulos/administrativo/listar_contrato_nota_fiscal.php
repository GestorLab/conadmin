<?
	...
	$filtro_nota_fiscal				= $_POST['filtro_nota_fiscal'];
	...
	if($filtro_nota_fiscal != ''){
		switch($filtro_nota_fiscal){
			case '1':
				$filtro_url .= "&NotaFiscal=".$filtro_nota_fiscal;
				$filtro_sql .=" and ( LocalCobranca.IdNotaFiscalTipo is null and
			...
			case '2':
				//echo "Geração";
				$filtro_url .= "&NotaFiscal=".$filtro_nota_fiscal;
				$filtro_sql .= " and LocalCobranca.IdNotaFiscalTipo is not null";
				break;
			case '3':
				//echo "Quitação";
				$filtro_url .= "&NotaFiscal=".$filtro_nota_fiscal;
				$filtro_sql .= " and Contrato.NotaFiscalCDA = 1";
				break;
		}
	}
	...
	echo "<?xml-stylesheet type=\"text/xsl\" href=\"listar_contrato_nota_fiscal_xsl.php$filtro_url\"?>";
	echo "<db>";
	...
	while($lin = mysql_fetch_array($res)){
		...
		switch($lin[NotaFiscalCDA]){
			case '1':
				$NotaFiscalCDATemp = "Sim";
				$NotaFiscalCDA = $lin[NotaFiscalCDA];
				break;
			case '2':
				$NotaFiscalCDATemp = "Não";
				$NotaFiscalCDA = $lin[NotaFiscalCDA];
				break;
		}
		...
		if($lin[TipoPessoa] == '1'){
			$TipoPessoa		= $lin[TipoPessoa];
			$TipoPessoaTemp	= "Juridica";
		}else{
			$TipoPessoa		= $lin[TipoPessoa];
			$TipoPessoaTemp	= "Fisica";	
		}
		if(($lin[IdNotaFiscalTipo] == "NULL") && ($lin[NotaFiscalCDA] != 1)){
			$NotaFiscal = "Eventual";
		}
		if($lin[IdNotaFiscalTipo] != "" || $lin[IdNotaFiscalTipo] != "NULL"){
			$NotaFiscal = "Geração";
		}
		if($lin[NotaFiscalCDA] == 1){
			$NotaFiscal = "Quitação";
		}
		switch($filtro_nota_fiscal){
			case '1':
		...
		echo 	"<TipoPessoa><![CDATA[$TipoPessoa]]></TipoPessoa>";
		echo 	"<TipoPessoaTemp><![CDATA[$TipoPessoaTemp]]></TipoPessoaTemp>";
		...
		echo 	"<NotaFiscalCDA><![CDATA[$NotaFiscalCDA]]></NotaFiscalCDA>";
		echo 	"<NotaFiscalCDATemp><![CDATA[$NotaFiscalCDATemp]]></NotaFiscalCDATemp>";
		echo 	"<NotaFiscal><![CDATA[$NotaFiscal]]></NotaFiscal>";
